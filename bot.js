...
var captionPhrase = [
  ...
  "Trying to find the nearest washroom in public"
];

var people = ["Albert", "John", "Harold"];

//Random server Messages
client.on("message", (msg) => {
  //caption me
  if (
    msg.content.toLowerCase() === "caption me!" ||
    msg.content.toLowerCase() === "caption me"
  ) {
    //Albert
    if (msg.author.username === "Alobyte") {
      getFolderSize("Albert").then(function (response) {
        ...
        msg.reply(captionPhrase[randomNum(captionPhrase)], {
          files: [
            "./captionAlbert/" + Math.floor(Math.random() * exAlbert) + ".jpg",
          ],
        });
      });
    }
    ...
    if (msg.author.username === "John") {
      getFolderSize("John").then(function (response) {
        ...
        msg.reply(captionPhrase[randomNum(captionPhrase)], {
          files: [
            "./captionJohn/" + Math.floor(Math.random() * exJohn) + ".jpg",
          ],
        });
      });
    }
    ...
    if (msg.author.username === "Harold") {
      getFolderSize("Harold").then(function (response) {
        ...
        msg.reply(captionPhrase[randomNum(captionPhrase)], {
          files: [
            "./captionHarold/" + Math.floor(Math.random() * exHarold) + ".jpg",
          ],
        });
      });
    }
      ...
      msg.reply(captionPhrase[randomNum(captionPhrase)], {
        files: [
          "./caption" +
            randomPerson +
            "/" +
            Math.floor(Math.random() * exPerson) +
            ".jpg",
        ],
      });
    });
